...
describe "FindFlight" do
  list = '../lib/Simple-input.csv'
    ...
    converted_list = @flight_finder.find_all_durations(@flight_finder.convert_price_to_integer(@flight_finder.convert_time_to_integer(@travel_finder.delete_whitespace(@travel_finder.get_flights(list)))))
  ...
  it "adds indirect flights" do
	...
	it "compares direct times with indirect times" do
    converted_list = @flight_finder.find_all_durations(@flight_finder.convert_price_to_integer(@flight_finder.convert_time_to_integer(@travel_finder.delete_whitespace(@travel_finder.get_flights(list)))))
		direct_flights = @flight_finder.find_direct_flights(converted_list)
		shortest_or_cheapest = @flight_finder.shortest_or_cheapest_direct(direct_flights, @flight_finder.check_direct_duration(direct_flights))
		...
		expect(@flight_finder.pick_for_jen(shortest_or_cheapest, indirect_flight_duration, combined_itinerary).price).to eq(300)
	end
end